...
export const validateMedicationScheduleCreate = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (prescriptionId && !isValidUuid(prescriptionId)) {
    return res.status(400).json({ status: "error", message: "prescriptionId tidak valid", error_code: "VALIDATION_ERROR" });
  }

  if (isMissing(drugName)) {
    return res.status(400).json({ status: "error", message: "Nama obat wajib diisi", error_code: "VALIDATION_ERROR" });
  }

  if (isMissing(dosage)) {
    return res.status(400).json({ status: "error", message: "Dosis wajib diisi", error_code: "VALIDATION_ERROR" });
  }

  if (!Number.isInteger(frequency) || frequency < 1 || frequency > 3) {
    return res.status(400).json({ status: "error", message: "Frekuensi harus angka 1 sampai 3", error_code: "VALIDATION_ERROR" });
  }

  if (!validateScheduledTimes(scheduledTimes)) {
    return res.status(400).json({ status: "error", message: "scheduledTimes wajib array jam format HH:mm", error_code: "VALIDATION_ERROR" });
  }
