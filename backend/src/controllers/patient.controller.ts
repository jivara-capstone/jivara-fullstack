...
export const updatePatient = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(200).json({ status: "success", data: patient, message: "Data pasien berhasil diperbarui" });
  } catch (error) {
    sendError(res, error);
  }
};
...
export const assignPatient = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(200).json({ status: "success", data: patient, message: "Pasien berhasil ditugaskan ke perawat" });
  } catch (error) {
    sendError(res, error);
  }
};
...
export const deactivatePatient = async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.status(200).json({ status: "success", message: "Patient deactivated successfully" });
  } catch (error) {
    sendError(res, error);
  }
};
