...
const hasTerminalLog = async (job: typeof medicationReminderJobs.$inferSelect) => {
  const rows = await db
    .select({ id: medicationLogs.id, status: medicationLogs.status })
    .from(medicationLogs)
    .where(and(
...
const sendUrgentEscalation = async (job: typeof medicationReminderJobs.$inferSelect, schedule: typeof medicationSchedules.$inferSelect) => {
  ...
  await db
    ...
    .set({
      ...
      updatedAt: new Date(),
      lastError: result.sent > 0 ? null : "Urgent reminder gagal dikirim",
    })
    .where(eq(medicationReminderJobs.id, job.id));
};
...
const processEscalations = async (now: Date) => {
  const jobs = await db
    ...
    .from(medicationReminderJobs)
    .where(and(
      lte(medicationReminderJobs.scheduledTime, new Date(now.getTime() - URGENT_AFTER_MINUTES * 60_000)),
    ))
    .limit(100);
